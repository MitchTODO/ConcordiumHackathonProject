...
import { sha256,getTxStatus } from "./Utils/WillUtils.js";
...
function Notary(props) {
    ...
    useEffect(() => {
        ...
        .then(c =>  {
            ...
            let contractService = new ContractServices(c)
    ...
    const getWills = async () => {
        ...
        contractService.getWills(account,testator,parseInt(willId))
        .then(willObject => {
        setIsLoading(false);
    ...
    function notarize() {
        setUploading(true)
        contractService.notarize(account,willHash,willId,testator,witnessAddress)
        .then(txHash => {
            getTxStatus(client,txHash).then(result => {
                // tx was successful 
                if(result) {
    ...
    return (
        <div className="m-5">
            <button className="btn btn-primary" onClick={connectId}> {account == "" ? "Connect": account}</button>
            ...
            { will != null &&
                <div className="container bg-secondary rounded m-3 ">
                    ...
                    <div className="container bg-white rounded mt-3 p-4">
                        <p className="ard-text black-text text-small indent">{willFile}</p>
                    </div>
